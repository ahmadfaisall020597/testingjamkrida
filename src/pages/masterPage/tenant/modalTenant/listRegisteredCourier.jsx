...
import { setListRegisteredCourier, setListUser, setShowModal } from "../TenantSlice";
import { getAllActiveUsers } from "src/utils/api/apiMasterPage";
import { toast } from "react-toastify";

// eslint-disable-next-line no-unused-vars
export default function ModalContentListRegisteredCourier({ tenantId, canEdit }) {
  const dispatch = useDispatch();
  const { listUser, listRegisteredCourier } = useSelector((state) => state.tenant);
  const [tempListRegisteredCourier, setTempListRegisteredCourier] = useState([]);
  ...
  const fetchData = useCallback(() => {
    ...
    getAllActiveUsers(params).then(res => {
      const filteredData = res.data.data.filter(user => !tempListRegisteredCourier.some(r => r.USER_ID == user.id));
      setTotalRows(res.data.totalData - tempListRegisteredCourier.length);
      dispatch(setListUser(filteredData));
      setLoading(false);
    }).catch(() => {
      setLoading(false);
    });
  }, [currentPage, dispatch, searchParams, sortConfig, tempListRegisteredCourier]);
  ...
  useEffect(()=>{
    setTempListRegisteredCourier(listRegisteredCourier)
  ...
  const handleAdd = (user) => {
    if (!tempListRegisteredCourier.some((u) => u.USER_ID == user.id)) {
      const translate = {
        USER_ID: user.id,
      ...
      setTempListRegisteredCourier([...tempListRegisteredCourier, translate]);
      // dispatch(setListUser(listUser.filter((u) => u.id !== user.id)));
    }else{
  ...
  const handleDelete = (user) => {
    setTempListRegisteredCourier(tempListRegisteredCourier.filter((u) => u.USER_ID != user.USER_ID));
    // dispatch(setListUser([...listUser, user]));
  };

  const handleSubmit = () => {
    dispatch(setListRegisteredCourier(tempListRegisteredCourier));
      ...
      <Col sm={6}>
        <Row className="row-modal-section-registered-user px-3 mb-3">
          <Col className="bg-warning">
            <h5 className="m-0 my-2 mx-2">Registered Courier</h5>
          </Col>
        </Row>
        <div>
          <TableComponent
            columns={canEdit ? columnListRegisteredUser: columnsConfigMaster}
            data={tempListRegisteredCourier}
            responsive
            striped
            persistTableHead
            pagination
