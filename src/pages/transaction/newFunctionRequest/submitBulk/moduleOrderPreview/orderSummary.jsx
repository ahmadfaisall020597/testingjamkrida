...
const OrderSummary = ({listDataPackage, listDataProduct, loadingProduct, loadingPackage, roomSelectionData, productSelectionData, buttonNext, buttonPrevious, currentIndex, localState}) => {
  return (
    <>
      <Stack direction="horizontal" className="justify-content-between flex-wrap" gap={4}>
        <div className="my-1 px-3 py-2 header-title flex-fill">
        ...
        <Stack direction="horizontal" className="fs-5" gap={2}>
          <div role="button" onClick={()=> buttonPrevious()}><FaCaretSquareLeft size={25}/></div>
          <div>Page {currentIndex+1} of {localState?.length}</div>
          <div role="button" onClick={() => buttonNext()}><FaCaretSquareRight size={25}/></div>
        </Stack>
      </Stack>
