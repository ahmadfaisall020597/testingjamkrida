...
import { fnGetListProdukPenjaminan } from "./penjaminanFn";
import snkBankDKI from "src/assets/image/snk-bank-dki.png";
import { templateFileBase64DataPenjaminan } from "src/utils/dummy/templateFileBase64DataPenjaminan";
import "./style.scss";
...
import { storeBulk } from "../../../utils/api/apiPenjaminan";

const templateBase64 = templateFileBase64DataPenjaminan.excelFormPenjaminan;
const downloadTemplateFileName = "template_form_penjaminan_mitra.csv";
...
*/
export default function PenjaminanMitraUploadExcelPage() {
	...
	useEffect(() => {
		...
		// getSettingPerykeyAndMitra(module, selectedMitra, itemId);
	}, [])
	const penjaminanSelector = useSelector(state => state.penjaminan);
	console.log(penjaminanSelector)
	...
	const handleChange = async (key, value) => {
		...
		const response = await getSettingPerykeyAndMitra("PENJAMINAN_SETTINGS", entityName, value);
		const lampiranValues = response.data.data && response.data.data.length > 0
			? response.data.data.map(item => item.lampiran)
			: [];
	...
	return (
		<Container fluid id="penjaminan-mitra-upload-excel-page">
			<CardComponent title="Upload Excel Penjaminan" type="create">
