...
import { addCommentAndReview, getCommentAndReviewByProductId } from "src/utils/api/apiTransaction";
import { toast } from "react-toastify";
import { validateAndBuildReviewPayload } from "./requestCommentAndReviewFn";
...
const RateRequestCommentAndReview = () => {
  ...
  const fetchData =useCallback(() => {
    ...
    if(data === null){
      ...
      getCommentAndReviewByProductId(param).then((res) => {
        const response = res?.data?.data
        if(response.RATED && response.RATED > 0 ){
          setCanEdit(false)
        }else{
          setCanEdit(true)
        }
            ...
            <Col xl={6} className="pe-xl-4 create-col border-end border-3">
              ...
              <Stack direction="horizontal" className="mt-3 w-100 flex-wrap justify-content-between" gap={2}>
                ...
                <Stack direction="horizontal" gap={3} role="button" onClick={() =>{
                    if(!canEdit) return
                    handleChange("IS_FAVOURITE", !value.IS_FAVOURITE)
                  }}
                >
                  <p className="m-0 fw-bold">Set As Favourite</p>
              ...
              <Stack gap={2}>
                ...
                <Stack direction="horizontal" className="justify-content-end" gap={3}>
                  ...
                  <ButtonComponent
                    ...
                    onClick={() => History.navigate(objectRouterInquiry.requestCommentAndReview.path)}
                    title="Cancel"
                  />
                </Stack>
              </Stack>
...
export default RateRequestCommentAndReview;
